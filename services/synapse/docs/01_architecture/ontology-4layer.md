# 5계층 온톨로지 아키텍처 상세 (구 4계층에서 확장)

## 이 문서가 답하는 질문

- 5계층 온톨로지(구 4계층 + Driver)의 각 계층은 무엇을 표현하는가?
...
### 1.1 왜 5계층인가? (구 4계층에서 확장)

> **변경 이력**: 초기 설계(ADR-002)는 4계층(Resource-Process-Measure-KPI)이었으나, Vision 서비스의 인과 분석(Granger/VAR) 결과를 온톨로지에 반영하기 위해 **Driver 계층**이 추가되었다. 코드 기준: `ontology_service.py`의 `VALID_LAYERS = {"kpi", "measure", "process", "resource", "driver"}`
...
4. 인과 분석에서 **영향 요인(Driver)**이 도출되며 (환율변동, 수요변동, 유가변동)
5. 이 지표들과 Driver가 **전략적 성과**를 결정한다 (프로세스 효율성, ROI)
...
EventStorming은 도메인 이벤트를 중심으로 비즈니스 프로세스를 모델링하는 기법이다. EventStorming의 각 구성요소는 5계층 온톨로지에 자연스럽게 매핑된다.
...
```
{출하됨} ──produces──▶ [배송완료율: 95.2%] ──derived_from (KPI←Measure)──▶ (물류 효율성 KPI)
{결제됨} ──produces──▶ [일매출: 1.2억]     ──derived_from (KPI←Measure)──▶ (수익성 KPI)
```
...
### 2.4 Driver 계층 (인과 영향 요인)

> "KPI에 영향을 미치는 외부/내부 인과 요인"

Driver 계층은 Vision 서비스의 인과 분석(Granger/VAR) 결과로 자동 생성되는 노드이다. Measure와 KPI 사이에 위치하며, `CAUSES`/`INFLUENCES` 관계로 연결된다.

#### 노드 유형

| 노드 유형 | Neo4j 레이블 | 주요 속성 | 설명 |
|----------|------------|----------|------|
| **Driver** | `:Driver` | name, type, weight, lag, confidence, method, direction | 인과 분석 결과 자동 생성 |

#### 주요 예시

- 환율변동 (Exchange Rate Fluctuation)
- 수요변동 (Demand Variation)
- 유가변동 (Oil Price Change)
- 원자재 가격 변동

#### 관계 타입

```
(Driver)-[:CAUSES {weight, lag, confidence, method, direction}]->(KPI)
(Driver)-[:INFLUENCES {weight, lag, confidence, method, direction}]->(Measure)
(Measure)-[:CAUSES]->(Driver)  // 역방향 인과
```

#### 생성 흐름

```
Vision 인과 분석 (Granger/VAR)
  │
  ▼ POST /api/v3/synapse/ontology/relations:bulk
  │
  ▼ Synapse ontology_service.bulk_update_relations()
  │
  └─ Driver 노드 자동 생성 + CAUSES/INFLUENCES 관계 설정
```
...
| 매핑 규칙 | 설명 |
|----------|------|
| Measure 집계 → KPI | 여러 Measure 노드로부터 KPI가 DERIVED_FROM 관계로 파생됨 |
...
// KPI ← Measure: KPI가 Measure로부터 파생됨
MATCH (m:Throughput:Measure {case_id: $case_id})
MATCH (k:ProcessEfficiency:KPI {case_id: $case_id})
CREATE (k)-[:DERIVED_FROM {weight: 1.0, formula: 'numerator'}]->(m)

// KPI ← Measure: KPI가 다른 Measure에도 의존
MATCH (k:ProcessEfficiency:KPI {case_id: $case_id})
MATCH (m:ResourceUtilization:Measure {case_id: $case_id})
CREATE (k)-[:DERIVED_FROM {formula: 'denominator'}]->(m)
```
...
(Resource)-[:SUPPORTS {contribution_type}]->(Measure)
(Process)-[:PRODUCES {calculation}]->(Measure)
(KPI)-[:DERIVED_FROM {weight, formula}]->(Measure)
(Process)-[:INFLUENCES {impact_type, strength}]->(KPI)
(Driver)-[:CAUSES {weight, lag, confidence, method, direction}]->(KPI)
(Driver)-[:INFLUENCES {weight, lag, confidence, method, direction}]->(Measure)
(Measure)-[:CAUSES {weight, lag, confidence, method}]->(Driver)
```
...
```
Company ──PARTICIPATES_IN──▶ DataCollection ──PRODUCES──▶ Revenue ◀──DERIVED_FROM── ProcessEfficiency
   │                              │                             │                                    │
...
MATCH path = (a)-[:PARTICIPATES_IN|PRODUCES|DERIVED_FROM|INFLUENCES*..4]->(k:KPI)
...
WHERE n:Resource OR n:Process OR n:Measure OR n:Driver OR n:KPI
OPTIONAL MATCH (n)-[r]->(m {case_id: $case_id})
WHERE m:Resource OR m:Process OR m:Measure OR m:Driver OR m:KPI
RETURN n, r, m
```
...
[Driver 계층]
  ├─ (:Driver {name: "환율변동", type: "external", weight: 0.72, lag: 30, confidence: 0.85, method: "granger"})
  ├─ (:Driver {name: "수요변동", type: "external", weight: 0.65, lag: 14, confidence: 0.78, method: "var"})
  ├─ (:Driver {name: "유가변동", type: "external", weight: 0.58, lag: 45, confidence: 0.82, method: "granger"})
  ├─ (:Driver {name: "원자재 가격 변동", type: "external", weight: 0.61, lag: 21, confidence: 0.80, method: "var"})
  └─ (:Driver {name: "생산 설비 가동률", type: "internal", weight: 0.88, lag: 7, confidence: 0.91, method: "granger"})
...
pm4py로 발견된 프로세스 모델은 5계층 온톨로지에 자동으로 반영된다. 이 연동을 통해 설계(EventStorming)와 실행(이벤트 로그)의 차이를 온톨로지 그래프 위에서 직접 분석할 수 있다.
...
이벤트 로그 인제스트
    ...
    └─ Conformance Checking
           │
           ▼ fitness/precision → KPI 계층 (ProcessEfficiency)에 DERIVED_FROM 관계
```
...
| 결정 | 근거 |
|------|------|
| 5계층 구조 채택 (구 4계층 + Driver) | K-AIR 온톨로지 설계를 확장하여 인과 분석 결과 반영 (ADR-002 Superseded) |
