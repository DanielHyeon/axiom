...
| Method | Path | 설명 |
|--------|------|------|
| POST | `/documents/{doc_id}/extract-ontology` | 비동기 추출 작업 시작 |
| GET | `/documents/{doc_id}/ontology-status` | 추출 진행 상태 |
| GET | `/documents/{doc_id}/ontology-result` | 추출 결과 조회 |
| PUT | `/ontology/{entity_id}/confirm` | HITL: 개별 개체 확인/수정 |
| POST | `/cases/{case_id}/ontology/review` | HITL: 일괄 검토 |
| GET | `/cases/{case_id}/review-queue` | HITL: 검토 대기열 |
| POST | `/documents/{doc_id}/retry` | 실패한 추출 재시도 |
| POST | `/documents/{doc_id}/revert-extraction` | Saga 보상: 추출 결과 되돌리기 |
