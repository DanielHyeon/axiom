"""Weaver 서비스 이벤트 계약 레지스트리 (DDD-P3-01).

Weaver가 소유(발행)하는 도메인 이벤트 2종의 스키마·버전·멱등성 규칙을 정의한다.
"""
...
EVENT_CONTRACTS: dict[str, EventContract] = {
    "METADATA_SYNC_COMPLETED": EventContract(
        event_name="METADATA_SYNC_COMPLETED",
        owner_service="weaver",
        version="1.0.0",
        payload_schema="weaver/metadata_sync_completed/v1",
        idempotency_key_rule="event_type:aggregate_id",
    ),
    "DATASOURCE_SCHEMA_CHANGED": EventContract(
        event_name="DATASOURCE_SCHEMA_CHANGED",
        owner_service="weaver",
        version="1.0.0",
