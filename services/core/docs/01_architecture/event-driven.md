...
> 역할별 기본 구독의 전체 매핑은 `canvas/docs/04_frontend/watch-alerts.md` §8을 참조한다.
...
> Watch CEP Worker의 주기적 스캔(5분 간격)으로 미확인 CRITICAL 알림을 탐지한다. 에스컬레이션 알림 자체도 idempotency_key로 중복 방지된다. 상세 정책은 `canvas/docs/04_frontend/watch-alerts.md` §9를 참조한다.
...
| 문서 | 관계 |
|------|------|
| `canvas/docs/04_frontend/watch-alerts.md` | 프론트엔드 알림 UI — 역할별 알림 관련성(§8), 생명주기 & 에스컬레이션(§9), 실시간 채널 아키텍처(§10) |
