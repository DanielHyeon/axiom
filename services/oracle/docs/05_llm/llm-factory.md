...
`llm_factory.py`는 LLM 프로바이더를 통일된 인터페이스로 추상화한다.

> **현재 구현 상태**: MockLLM(스마트 모드)만 등록되어 있으며, OpenAI/Google 클라이언트는 미구현이다. MockLLM은 한국어 자연어 질문을 파싱하여 실제 실행 가능한 SQL을 생성한다. 프로덕션 배포 시 OpenAI 등을 `_registry`에 추가하면 된다.
