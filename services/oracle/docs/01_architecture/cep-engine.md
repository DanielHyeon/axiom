...
| 단계 | 작업 | 상태 |
|------|------|------|
| 1 | Oracle 내 SimpleCEP를 그대로 이식 (호환성 유지) | 불필요 (직접 프록시 방식 채택) |
| 2 | Core Watch 모듈 기본 구조 설계 | 완료 |
| 3 | 이벤트 룰 API를 Core Watch로 이동 | 완료 (Core `/api/v1/watches/*`) |
| 4 | Oracle의 이벤트 API를 Core Watch proxy로 전환 | **완료** (`app/api/events.py`) |
| 5 | Oracle 내 SimpleCEP 코드 제거 | 완료 (SimpleCEP 없음, 프록시만 존재) |

> **현재 구현**: Oracle의 `/text2sql/events/*` 엔드포인트는 모두 Core `/api/v1/watches/*`를 httpx로 프록시한다. Oracle 자체에는 이벤트 엔진이 존재하지 않으며, `_core_request()` 헬퍼로 모든 요청을 Core에 위임한다. `/text2sql/watch-agent/chat` 엔드포인트는 Core `/api/v1/agents/chat`을 호출하고, `_propose_watch_rule()` 함수로 자연어 메시지 기반 감시 룰을 제안한다.
