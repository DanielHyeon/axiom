...
async def create_scenario(
    ...
    scenario = await svc.create("default", body)
...
async def list_scenarios(
    ...
    svc = ScenarioService(session)
    items = await svc.list_scenarios("default", workspace_id)
...
async def get_scenario(
    ...
    svc = ScenarioService(session)
    scenario = await svc.get("default", scenario_id)
    if not scenario:
...
async def get_run(
    ...
    svc = ScenarioService(session)
    run = await svc.get_run("default", run_id)
    if not run:
