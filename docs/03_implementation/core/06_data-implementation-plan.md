...
6. Watch Early Warning 폐루프 테이블(`watch_alert_actions`) 및 SLA 지표 컬럼(`resolved_at`, `false_positive`) 반영
7. 알림 등급(`LOW/MEDIUM/HIGH/CRITICAL`) 표준과 API/DB enum 정합성 점검

## 5. 통과 기준 (Gate 06)
- 무결성 제약 위반 0건
- 마이그레이션 드라이런/롤백 리허설 완료
- 핵심 조회 성능 목표(p95) 충족
- 탐지→확인 `p95 <= 15분`, 탐지→해결 `p95 <= 24시간` 지표 산출 가능
- 오탐률(`false_positive`) 월간 집계 쿼리/대시보드 연결 확인
