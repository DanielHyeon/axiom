# WEAVER 01_architecture 아키텍처 구현 계획

## 1. 문서 목적
- weaver 프로젝트의 01_architecture 설계 문서를 실제 구현 백로그로 변환한다.
- 단계별 책임 에이전트와 통과 기준을 명확히 정의한다.

## 2. 참조 설계 문서
- services/weaver/docs/01_architecture/adapter-pattern.md
- services/weaver/docs/01_architecture/architecture-overview.md
...
6. Semantic Layer 책임 경계(Weaver/Synapse/Vision) 문서-구현 매핑 확정
7. Legacy Data Isolation Policy 준수 점검 항목(원본 DB 불변/읽기전용)을 아키텍처 체크리스트에 반영

## 5. 통과 기준 (Gate 01)
- 금지 의존(역방향 import, 우회 호출) 0건
- 핵심 시퀀스별 책임 컴포넌트가 단일화됨
- ADR와 구현 계획 간 충돌 0건
- 원본 레거시 스키마 직접 변경 경로 0건
