# WEAVER 99_decisions ADR/의사결정 반영 계획

## 1. 문서 목적
- weaver 프로젝트의 99_decisions 설계 문서를 실제 구현 백로그로 변환한다.
- 단계별 책임 에이전트와 통과 기준을 명확히 정의한다.

## 2. 참조 설계 문서
- services/weaver/docs/99_decisions/ADR-001-mindsdb-gateway.md
- services/weaver/docs/99_decisions/ADR-002-adapter-pattern.md
- services/weaver/docs/99_decisions/ADR-003-neo4j-metadata.md
- services/weaver/docs/99_decisions/ADR-004-metadata-service.md
...
6. `Implemented/Experimental/Planned` 상태 라벨과 실제 구현 상태 동기화 규칙 관리

## 5. 통과 기준 (Gate 99)
- ADR별 구현 항목 추적 가능(링크/ID)
- 상충 결정 미해결 항목 0건
- 재평가 트리거/책임자/주기 명시 완료
- 정책 문서(`legacy-data-isolation-policy`)와 ADR 간 충돌 0건
