# WEAVER 02_api API 계약 구현 계획

## 1. 문서 목적
- weaver 프로젝트의 02_api 설계 문서를 실제 구현 백로그로 변환한다.
- 단계별 책임 에이전트와 통과 기준을 명확히 정의한다.

## 2. 참조 설계 문서
- services/weaver/docs/02_api/datasource-api.md
- services/weaver/docs/02_api/metadata-api.md
...
6. `metadata-catalog-api` 상태 태깅(`Planned/Experimental`)과 런타임 구현 상태 동기화 규칙 확정

## 5. 통과 기준 (Gate 02)
- OpenAPI/계약 문서와 구현 파라미터 불일치 0건
- 4xx/5xx 에러 코드 표준화 완료
- 권한 누락 엔드포인트 0건
- 상태 표기(`Implemented/Experimental/Planned`)와 실제 구현 상태 불일치 0건
