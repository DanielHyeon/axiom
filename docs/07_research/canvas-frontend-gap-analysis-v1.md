...
### 8.6 Phase F 총 규모 (리뷰 수정 반영)

| 항목 | 수정 전 | 수정 후 | 비고 |
|------|:------:|:------:|------|
| 신규 파일 | ~36 | **~20** | KG-3(워크플로) -9, KG-8(CEP) -2 |
| 수정 파일 | ~8 | **~10** | 기존 파일 수정으로 전환 |
| 예상 LOC | ~5,240 | **~3,190** | KG-3 -760, KG-5 -200, KG-8 -300 |
| 추가 패키지 | `bpmn-js`, `@xyflow/react` | **`bpmn-js`만** | 워크플로는 기존 Cytoscape 사용 |
...
| **Phase F (KAIR 이식)** | **~20** | **~10** | **~3,190** | **DMN + 캘린더 + BPMN + 부분갭 (워크플로/CEP 이미 구현)** |
| **합계** | **~66** | **~74** | **~10,990** | |
...
| ARIA 속성 사용 | 55건 | **200건+** |
| KAIR 기능 커버리지 | 7% 완전 + 80% 부분 | **80%+ 완전** |
| KAIR 전체 미구현 기능 | 2건 (DMN/캘린더) | **0건** |

---

> **문서 끝** — v1.4. 2026-03-24 기준 Canvas 프론트엔드 현황 분석, 구현 계획, UI/UX 품질 개선, KAIR 대비 갭 분석.
> **v1.4 리뷰 수정**: workflow-editor(15파일/1,768 LOC) + CEP(4파일/765 LOC) 실 구현 확인.
> 전체 미구현 4건→2건, Phase F 규모 ~5,240→~3,190 LOC 축소. 스키마 캔버스 듀얼 모드 이미 존재 확인.
> 총 구현 규모 ~10,990 LOC (Phase A-F 전체).
