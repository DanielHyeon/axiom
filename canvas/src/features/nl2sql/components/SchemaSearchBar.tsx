...
export function SchemaSearchBar({
  ...
  return (
    <div ref={containerRef} className="relative px-3 py-2 border-b border-border">
      ...
      {/* 검색 결과 드롭다운 */}
      {showDropdown && (
        <div className="absolute left-3 right-3 top-full mt-1 bg-card border border-border rounded shadow-lg z-50 max-h-[200px] overflow-y-auto">
          {results.map((result, idx) => (
            ...
            >
              ...
              <span className="ml-auto text-[9px] text-foreground/25 font-mono shrink-0">
                {result.schema}
              </span>
            </button>
          ))}
