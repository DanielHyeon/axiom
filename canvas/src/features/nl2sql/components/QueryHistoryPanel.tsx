...
export function QueryHistoryPanel({ datasourceId, onSelect }: QueryHistoryPanelProps) {
 ...
 <div className="h-3 bg-border rounded w-1/2 mt-2" />
 </div>
 ))}
 </div>
 );
 ...
 <p className="text-xs text-foreground/60 font-mono">{t('nl2sqlExt.historyLoadError')}</p>
 </div>
 );
 }
 ...
 return (
 <div className="py-8 text-center">
 <p className="text-xs text-foreground/60 font-mono">{t('nl2sqlExt.noRecentQueries')}</p>
 </div>
 );
 }
 ...
 className={cn(
 'w-full text-left py-3 px-6 transition-colors hover:bg-muted',
 idx === 0 && 'bg-muted'
 )}
 >
 <span
 className={cn(
 'block text-[13px] font-heading line-clamp-2',
 idx === 0 ? 'text-foreground font-medium' : 'text-muted-foreground'
 )}
 >
 {item.question}
 </span>
 <span className="mt-1.5 flex items-center gap-2 text-[11px] text-foreground/60 font-mono">
 {item.created_at && (
