import { Link } from 'react-router-dom';
import { ROUTES } from '@/lib/routes/routes';
import { List, MessageSquare, BarChart3 } from 'lucide-react';
import { useTranslation } from 'react-i18next';

const ACTIONS = [
 { labelKey: 'caseDashboardExt.caseList', to: ROUTES.CASES.LIST, icon: List },
 { labelKey: 'caseDashboardExt.nl2sqlLabel', to: ROUTES.ANALYSIS.NL2SQL, icon: MessageSquare },
 { labelKey: 'caseDashboardExt.olapPivot', to: ROUTES.ANALYSIS.OLAP, icon: BarChart3 },
];

export function QuickActionsPanel() {
 const { t } = useTranslation();
 return (
 <div className="glass-card rounded-xl p-5">
 <h3 className="mb-3 text-[13px] font-semibold text-foreground">{t('caseDashboardExt.quickActions')}</h3>
 <div className="flex flex-wrap gap-2">
 {ACTIONS.map((a) => (
 <Link
 key={a.to}
 to={a.to}
 className="inline-flex items-center gap-2 rounded-lg bg-muted/40 border border-border/30 px-4 py-2.5 text-[13px] font-medium text-foreground transition-all duration-200 hover:bg-primary/10 hover:text-primary"
 >
 <a.icon className="h-4 w-4 text-primary/70" />
 {t(a.labelKey)}
 </Link>
 ))}
 </div>
 </div>
 );
}
