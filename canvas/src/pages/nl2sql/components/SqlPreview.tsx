...
export function SqlPreview({ sql, onRun }: SqlPreviewProps) {
 ...
 return (
 <div className="border border-border rounded-md overflow-hidden bg-card my-3">
 <div className="flex items-center justify-between px-3 py-1.5 bg-muted border-b border-border">
 <span className="text-xs font-mono text-foreground/60">SQL</span>
 <div className="flex space-x-1">
 <Button variant="ghost" size="icon" className="h-6 w-6" onClick={handleCopy} title={t('nl2sqlPage.copySql')}>
 {copied ? <Check className="h-3 w-3 text-success" /> : <Copy className="h-3 w-3 text-foreground/60" />}
 </Button>
 {onRun && (
 <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => onRun(sql)} title={t('nl2sqlPage.runSql')}>
