...
export function DroppableZone({ id, title, items, accepts, onRemove }: DroppableZoneProps) {
 ...
 <div className="w-24 pt-2 text-xs font-medium text-muted-foreground shrink-0">
 {title}
 </div>
 <div
 ref={setNodeRef}
 className={`${isOver ? 'bg-indigo-950/20 border-indigo-500/50' : 'bg-card border-border'} flex-1 min-h-[42px] border rounded-md p-1.5 flex flex-wrap gap-2 transition-colors`}
 >
 <SortableContext
...
function SortablePill({ id, item, onRemove }: { id: string, item: Dimension | Measure, onRemove: () => void }) {
 ...
 const bgClass = isMeasure ? 'bg-primary border-indigo-500' : 'bg-muted border-border';
 ...
 className="p-1 hover:bg-sidebar/20 rounded-sm transition-colors"
 >
 <X size={12} />
 </button>
 </div>
