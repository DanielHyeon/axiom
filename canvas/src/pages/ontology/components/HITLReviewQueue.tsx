...
const LAYER_BADGE: Record<string, string> = {
 ...
 measure: 'border-blue-300 text-primary',
...
export function HITLReviewQueue({
 ...
 {error && (
 <div className="m-3 rounded border border-red-200 bg-red-50 p-3 text-sm text-destructive">
 {error}
 </div>
 )}
 ...
 <div className="text-[13px] text-foreground truncate font-heading">
 ...
 LAYER_BADGE[item.node_layer] ?? 'border-border text-foreground/60',
 )}
 >
 {item.node_layer}
 </Badge>
 )}
 <span className="text-[10px] text-foreground/60 flex items-center gap-0.5 font-mono">
 ...
 >
 <MessageSquare
 className={cn('h-3.5 w-3.5', showComment ? 'text-primary' : 'text-foreground/60')}
 />
 </Button>
 </div>
 </div>
 ...
 variant="ghost"
 size="sm"
 className="w-full h-8 text-xs text-foreground/60 font-heading"
 disabled={loading}
