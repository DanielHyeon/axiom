...
export function ConceptMapView({ caseId }: ConceptMapViewProps) {
 ...
 {/* Relation type */}
 <div className="w-36">
 <label className="block text-[11px] text-foreground/60 font-mono mb-1">{t('ontologyPage.relationType')}</label>
 <select
 value={relType}
 ...
 {tables.map((tbl) => (
 <option key={`${tbl.datasource}.${tbl.schema}.${tbl.name}`} value={tbl.name}>
 {tbl.schema}.{tbl.name}
 </option>
 ))}
 </select>
 ...
 >
 {createMut.isPending ? '...' : t('ontologyPage.addBtn')}
 </button>
 </div>
 {createMut.isError && (
 ...
 <th className="text-left py-2 px-3 font-medium">{t('ontologyPage.layerHeader')}</th>
 <th className="text-left py-2 px-3 font-medium">{t('ontologyPage.relationHeader')}</th>
 <th className="text-left py-2 px-3 font-medium">{t('ontologyPage.msgb351a9b7')}</th>
 <th className="text-left py-2 px-3 font-medium">{t('ontologyPage.schemaHeader')}</th>
 <th className="text-left py-2 px-3 font-medium">{t('ontologyPage.createdAtHeader')}</th>
