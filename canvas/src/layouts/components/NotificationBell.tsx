...
export function NotificationBell() {
 ...
 className="p-4 border-b border-border/50 hover:bg-muted/50 transition-colors flex flex-col gap-1"
 >
 <div className="flex justify-between items-start gap-2">
 <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${alert.severity === 'critical' ? 'bg-destructive/20 text-destructive' :
 alert.severity === 'warning' ? 'bg-warning/20 text-warning' :
 'bg-primary/20 text-primary'
 }`}>
 {alert.severity.toUpperCase()}
 </span>
 <span className="text-[10px] text-foreground0 whitespace-nowrap">
 {new Date(alert.timestamp).toLocaleTimeString()}
 </span>
 </div>
 <h5 className="text-sm font-medium text-foreground mt-1">{alert.title}</h5>
