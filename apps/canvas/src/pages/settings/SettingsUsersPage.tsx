  ...
  if (!user) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-sm text-neutral-400">로그인된 사용자 정보가 없습니다.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-medium text-neutral-300">현재 사용자</h3>
        <ul className="border border-neutral-700 rounded divide-y divide-neutral-700">
          <li className="px-4 py-2 flex justify-between">
            ...
            <span className="font-mono text-sm text-neutral-100">{user.id}</span>
          </li>
          <li className="px-4 py-2 flex justify-between">
          ...
          <li className="px-4 py-2 flex justify-between">
            ...
            <span className="text-neutral-100">{user.role}</span>
          </li>
          <li className="px-4 py-2 flex justify-between">
            <span className="text-neutral-400">테넌트</span>
            <span className="font-mono text-sm text-neutral-100">{user.tenantId}</span>
          </li>
          {user.permissions?.length > 0 && (
            <li className="px-4 py-2 flex justify-between gap-4">
              <span className="text-neutral-400 shrink-0">권한</span>
              <span className="text-sm text-right break-all text-neutral-100">
          ...
          {user.caseRoles && Object.keys(user.caseRoles).length > 0 && (
            <li className="px-4 py-2 flex justify-between gap-4">
              <span className="text-neutral-400 shrink-0">케이스 역할</span>
              <span className="text-sm text-right text-neutral-100">
