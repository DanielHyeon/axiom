...
export const WatchAlerts: React.FC = () => {
 ...
 <p className="text-muted-foreground text-sm">Real-time complex event processing (CEP) early warnings.</p>
 </div>
 ...
 {alerts.map(alert => (
 <div key={alert.id} className={`p-4 border-l-4 rounded bg-background shadow-sm flex flex-col ${alert.level === 'CRITICAL' ? 'border-red-500' :
