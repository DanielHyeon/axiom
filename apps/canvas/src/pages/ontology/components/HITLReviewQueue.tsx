...
export function HITLReviewQueue({ caseId, onClose }: HITLReviewQueueProps) {
 ...
 LAYER_BADGE[item.node_layer] ?? 'border-[#E5E5E5] text-foreground/60',
 )}
 >
 {item.node_layer}
 </Badge>
 )}
 <span className="text-[10px] text-foreground/60 flex items-center gap-0.5 font-[IBM_Plex_Mono]">
 ...
 variant="ghost"
 size="sm"
 className="w-full h-8 text-xs text-foreground/60 font-[Sora]"
 disabled={loading}
