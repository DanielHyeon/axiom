...
export function NodeDetail({ onFindPath, onImpactAnalysis }: NodeDetailProps) {
 ...
 {/* Properties */}
 <div>
 <h3 className="text-[11px] font-semibold text-foreground/60 font-[IBM_Plex_Mono] uppercase tracking-wider mb-3">
 속성
 </h3>
 <div className="space-y-2 bg-[#F5F5F5] rounded p-3">
 {Object.entries(selectedNode.properties).map(([key, value]) => (
 <div key={key} className="flex justify-between text-[13px]">
 <span className="text-foreground/60 font-[IBM_Plex_Mono] capitalize">{key}</span>
 ...
 <h3 className="text-[11px] font-semibold text-foreground/60 font-[IBM_Plex_Mono] uppercase tracking-wider">
 연결
 </h3>
 ...
 <span className="text-foreground/60 min-w-[30px] text-xs font-[IBM_Plex_Mono]">
 {conn.direction === 'out' ? '→' : '←'}
 </span>
 <span className="flex-1 text-black truncate font-[Sora]" title={conn.label}>{conn.label}</span>
 <span className="text-[10px] text-foreground/60 bg-[#F5F5F5] px-1.5 py-0.5 rounded font-[IBM_Plex_Mono]">
 {conn.relation}
 </span>
 </div>
 ))}
